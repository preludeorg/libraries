...
echo "###########################################################################################################"
echo
echo "Will your computer quarantine a malicious ELF file?"
echo
echo "Malicious files are used to gain entry and conduct cyberattacks against corporate systems through seemingly"
echo "innocuous email attachments or direct downloads. For example - a malicious Linux executable was used by the Royal Ransomware"
echo "group during a ransomware attack in February 2023."
echo
echo "This test will attempt to download a malicious file to your disk - a defanged version of the Royal Ransomware"
echo "ELF binary - to see how your machine will respond. Downloading malicious files can open you up to further attacks,"
echo "so the ability to quickly detect and quarantine any potentially harmful files is an important part of maintaining a proper security posture."
echo
echo "This is a Verified Security Test (VST) Developed by Prelude Research Inc."
echo
echo "[+] Applicable CVE(s): CVE-2021-21974"
echo "[+] ATT&CK mappings: T1486"
echo
echo "###########################################################################################################"
echo
find_test
...
echo
if ( echo "100 9 17 18 105 127" | grep -w -q $test_result );then
    echo -e "${GREEN}[✓] Good job! Your computer detected and responded to a malicious ELF binary dropped on "
    echo -e "the disk${NC}"
elif [ $test_result -eq 101 ];then
    echo -e "${RED}[!] This test was able to verify the existence of this vulnerability on your machine, as well as drop"
    echo "a malicious ELF binary on the disk. If you have security controls in place that you suspect should"
