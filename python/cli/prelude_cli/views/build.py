...
def clone(controller):
    ...
    for test in controller.list_tests():
        code = controller.download_test(test_id=test['id'])
        with open(f'prelude/{test["filename"]}.go', 'wb') as test_code:
...
def create_test(controller, rule):
    ...
    controller.upload_test(test_id=test_id, code=template)
...
def save_test(controller, path):
    ...
    with open(path, 'r') as source_code:
        controller.upload_test(test_id=Path(path).name, code=source_code.read())
