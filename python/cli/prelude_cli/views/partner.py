...
@click.argument('partner',
              type=click.Choice([c.name.lower() for c in Control], case_sensitive=False))
...
def attach_partner(controller, partner_code, api, user, secret):
    ...
    with Spinner(description='Attaching partner'):
        data = controller.attach(partner_code=Control[partner_code.upper()].value, api=api, user=user, secret=secret)
...
def detach_partner(controller, partner_code):
    ...
    with Spinner(description='Detaching partner'):
        controller.detach(partner_code=Control[partner_code.upper()].value)
...
def partner_endpoints(controller, partner_code, platform, hostname, offset):
    ...
    with Spinner(description='Fetching endpoints from partner'):
        data = controller.endpoints(partner_code=Control[partner_code.upper()].value, platform=platform, hostname=hostname, offset=offset)
    print_json(data=data)
...
def partner_deploy(controller, partner_code, host_ids):
    ...
    with Spinner(description='Deploying probes to hosts'):
        data = controller.deploy(partner_code=Control[partner_code.upper()].value, host_ids=host_ids)
    print_json(data=data)
