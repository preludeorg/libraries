...
class BuildController:
    ...
    def download_test(self, test_id):
        res = requests.get(f'{self.account.hq}/build/source/{test_id}', headers=self.account.headers)
        if res.status_code == 200:
            return res.content
        raise Exception(res.text)

    @verify_credentials
    def upload_test(self, test_id, code):
        res = requests.post(f'{self.account.hq}/build/source/{test_id}', json=dict(code=code), headers=self.account.headers)
        if not res.status_code == 200:
            raise Exception(res.text)
