    ...
    def encode(self, obj):
        ...
        def sort_lists(item):
            if isinstance(item, list):
                return sorted((sort_lists(i) for i in item), key=lambda nm: nm[get_key(nm)]if isinstance(nm, dict) else get_key(nm))
...
def check_dict_items(expected, actual):
    ...
    expected = json.loads(json.dumps(expected, sort_keys=True, default=str, cls=SortedListEncoder))
    actual = json.loads(json.dumps(actual, sort_keys=True, default=str, cls=SortedListEncoder))
